...
class AttendanceLogService
{
    ...
    {
        return $this->log->with(['project', 'department', 'employee'])
        ->paginate(config("app.pagination_per_page", 10));
    }
    public function getAllToday()
    {
        return $this->log->where('date', Carbon::now()->format('Y-m-d'))
        ->with(['project', 'department', 'employee'])
    ...
    public function getFilterDateAndEmployee($request)
    {
        ...
        return $query->with(['project', 'department', 'employee', 'portal'])
        ->withTrashed()
        ->orderBy('created_at', 'DESC')
        ->paginate(config("app.pagination_per_page", 10));
    }
    ...
    {
        $attendances = AttendanceLog::whereIn('employee_id', $employeeId)
        ->paginate(config("app.pagination_per_page", 10));
        return $attendances;
    }
}
